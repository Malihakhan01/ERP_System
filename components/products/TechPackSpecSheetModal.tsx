...
import { Printer, X, Scissors, Layers, Ruler, CheckCircle2 } from "lucide-react";
...
export function TechPackSpecSheetModal({
  ...
  React.useEffect(() => {
    if (!isOpen) return;
    document.body.classList.add("print-techpack-active");
    ...
    return () => {
      document.body.classList.remove("print-techpack-active");
  ...
  return createPortal(
    <div
      id="tech-pack-print-portal"
      className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 md:p-6 bg-slate-900/70 backdrop-blur-xs print:p-0 print:bg-white print:static print:block print:inset-auto print:w-full print:m-0"
      onClick={(e) => {
        if (e.target === e.currentTarget) onClose();
      }}
    >
      <div className="relative w-full max-w-5xl max-h-[92vh] flex flex-col bg-white rounded-2xl border border-slate-200 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150 print:max-h-none print:h-auto print:shadow-none print:border-none print:rounded-none print:m-0 print:w-full print:overflow-visible">
        
        {/* Sticky Top Bar (Always Visible & Pinned) */}
        <div className="sticky top-0 z-30 shrink-0 flex items-center justify-between px-5 sm:px-6 py-3.5 border-b border-slate-200 bg-white/95 backdrop-blur-md shadow-xs no-print">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="text-sm sm:text-base font-bold text-slate-900 truncate">
                  Garment Tech Pack & Specification Sheet
                </h2>
              ...
              <p className="text-xs text-slate-500 truncate">
                Style: <strong className="text-slate-700 font-semibold">{product.name}</strong> • Category: {product.category || "Apparel"}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            >
              ...
              <span className="hidden sm:inline">Print Tech Pack (A4)</span>
              <span className="sm:hidden">Print</span>
            </button>
            <button
              type="button"
              onClick={onClose}
              className="h-8 w-8 rounded-lg border border-slate-200 bg-white hover:bg-slate-100 flex items-center justify-center text-slate-600 hover:text-slate-900 transition-colors cursor-pointer shadow-xs"
              title="Close (Esc)"
        ...
        {/* Scrollable Tech Pack Body */}
        <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-6 text-slate-800 text-xs print:overflow-visible">
        ...
        {/* Sticky Bottom Bar (Always Accessible) */}
        <div className="sticky bottom-0 z-20 shrink-0 flex items-center justify-between border-t border-slate-200 bg-slate-50 px-6 py-3 no-print">
          <div className="flex items-center gap-2 text-xs text-slate-500">
            <CheckCircle2 className="h-3.5 w-3.5 text-emerald-600" />
            <span>Spec Sheet Status: <strong className="text-slate-800 font-semibold">{product.bomStatus === "verified" ? "Verified & Approved" : "Draft Spec"}</strong></span>
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-1.5 rounded-lg border border-slate-300 bg-white text-xs font-semibold text-slate-700 hover:bg-slate-100 transition-colors cursor-pointer shadow-2xs"
            >
              Close Spec Sheet
            </button>
            <button
